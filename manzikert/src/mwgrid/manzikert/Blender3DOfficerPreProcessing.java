...
public final class Blender3DOfficerPreProcessing {
	...
	private static final Logger LOG =
		Logger.getLogger(Blender3DOfficerPreProcessing.class.getPackage().getName());
	private static final String TICK_FILEPATH = "I:/MWGrid/branches/ManzikertSP/environment/src/resources/environment/";
	private static final String TICK_FILENAME = "psm001trace.txt";
	private static final int MAX_AGENTS = 39100;
	private static final int TICKS = 12810;

	private BufferedWriter fOutputTickFile;

	public Blender3DOfficerPreProcessing() throws IOException {
					...
					for (int i = 1; i < MAX_AGENTS; i++) {
						if (ticks[0][i].locX != ticks[1][i].locX || ticks[0][i].locY != ticks[1][i].locY) {
							if (!ticks[0][i].equals(NULL_TICK)) {
								if (ticks[0][i].agentType == 1 || ticks[0][i].agentType == 2 || ticks[0][i].agentType == 4){
	...
	public static void main(final String[] pArguments) throws Exception, IOException {
		Blender3DOfficerPreProcessing thistest = new Blender3DOfficerPreProcessing();
	}
