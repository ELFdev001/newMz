...
public final class Blender3DPreProcessingTestHC {
	public class TickEntry {
		...
		public TickEntry(int pTickNo, int pAgentNo, int pAgentType, int pLocX, int pLocY, int pLocZ, float pDist) {
			this.tickNo = pTickNo;
	...
	public Blender3DPreProcessingTestHC() throws IOException {
		final TickEntry NULL_TICK = new TickEntry(0, 0, 0, 0, 0, 0, 0);
		...
		for (int tick = 1; tick <= TICKS; tick++) {
			...
			while (thistickno == tick) {
				...
				float pDist = Float.parseFloat(tokens[5]);
				int pLocZ = Integer.parseInt(tokens[6]);
				TickEntry thistick = new TickEntry(pTickNo, pAgentNo, pAgentType, pLocX, pLocY, pLocZ, pDist);
	...
	public static void main(final String[] pArguments) throws Exception, IOException {
		Blender3DPreProcessingTestHC thistest = new Blender3DPreProcessingTestHC();
	}
